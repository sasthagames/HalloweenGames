...
public class HomingMissleTimeTrail : MonoBehaviour
{
GameObject target;
    ...
    void Start()
    {
        target = GameObject.FindGameObjectWithTag("PlayerTimeTrail");
	...
	public void Explode()
	{
		...
		Debug.Log("HomingMissleTimeTrail" );
	...
	{		
		...
		if  (collision.tag == "PlayerTimeTrail") 
		{
			 Instantiate(explosion, transform.position, Quaternion.identity);			
		//	Playerhealth.instance.TakeDamage(2);	
		Debug.Log("PlayerTimeTrail" );		
			Destroy(gameObject);
